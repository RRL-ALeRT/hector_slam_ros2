...
public:
  ...
  : Node("world_info", options)
  {
    ...
    status_timer_ = create_wall_timer(std::chrono::milliseconds(10), std::bind(&WorldInfo::statusTimer, this));
  ...
  void receive_info(const world_info_msgs::msg::WorldInfo::SharedPtr msg)
  {
    ...
    if (info.type == "qr")
    {
      ...
      if (qr_dict.find(info.num) != qr_dict.end())
        if (qr_dict[info.num].count >= 100)
          return;

      try {
        transform_pose(info, "map");
      }
    ...
    if (info.type == "victim")
    {
      if (victim_dict.find(info.num) != victim_dict.end())
        if (victim_dict[info.num].count >= 100)
          return;
