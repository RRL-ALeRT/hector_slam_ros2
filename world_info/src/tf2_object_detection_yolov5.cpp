...
{
public:
  explicit DetectObjectTf2(const std::string& filteredNodeName, const std::string& depth_topic, const std::string& depth_info, const rclcpp::NodeOptions & options = rclcpp::NodeOptions())
  : Node(filteredNodeName, options),
...
private:
  ...
  void median_depth_xyz(const world_info_msgs::msg::BoundingBoxArray::SharedPtr msg)
  {
    ...
    {
      RCLCPP_WARN(get_logger(), "Waiting for depth camera info");
      return;
    }
    ...
    for(auto& boundingbox: msg->array)
    {
      ...
      x_max = std::min(static_cast<int>(boundingbox.cx + boundingbox.width / 2), frame.size().width - 1);
      y_max = std::min(static_cast<int>(boundingbox.cy + boundingbox.height / 2), frame.size().height - 1);
      ...
      {
        RCLCPP_WARN(get_logger(), "Previous scanned image tf2 couldn't be produced.");
        continue;
      }
...
int main(int argc, char **argv)
{
  ...
  std::string filteredNodeName = depth_topic + "_object_tf2"; // Initial node name with symbols
  ...
  rclcpp::spin(std::make_shared<DetectObjectTf2>(filteredNodeName, depth_topic, depth_info));
  rclcpp::shutdown();
  return 0;
}
